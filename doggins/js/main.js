...
fetch(BREEDS_URL)
    ...
    .then(data => {
        const breedsObject = data.message; // Turn the message into object
        const breedsArray = Object.keys(breedsObject); // Turn object into array
        // console.log(breedsObject)  // Check if it works
        for (let i = 0; i < breedsArray.length; i++) {
          const option = document.createElement('option'); // <option></option>
          option.value = breedsArray[i]; // < value='breed'>
          option.innerText = breedsArray[i]; // <option value='breed'>breed</option>
          select.appendChild(option); //adds current <> tag to the select box list of options
        }
    })

    // need a smurf
    select.addEventListener('change', event => {
      console.log(event.target.value) // check drill depth 1.2.3
...
const getDoggoImg = url => {
  fetch(url)
      ...
      .then(data => {
        ...
        // console.log(data.message) // check what comes back
      })
}
...
const doggoInfo = {
  ...
  assignMF() {
    ...
    console.log(x) // see what happens
    if (x) {
      this.MF = 'Female';
